...
-- 2) 직원의 사번, 이름, 부서명, 직급 조회(부서 없는 사람 포함)
...
SELECT EMP_NAME, DEPT_CODE FROM EMPLOYEE
WHERE DEPT_CODE IN (SELECT DEPT_ID FROM DEPARTMENT -- 다중행
                    WHERE LOCATION_ID = (SELECT LOCAL_CODE FROM LOCATION
...
-- 3) 서브쿼리 결과를 이용해 메인쿼리 해석중인 1행을 대상으로 조회
...
-- (스칼라 + 상관 쿼리)
SELECT EMP_ID, EMP_NAME, MANAGER_ID, NVL((SELECT EMP_NAME FROM EMPLOYEE
                                      WHERE EMP_ID = E1.MANAGER_ID), '없음') 관리자명
                                    -- 메인쿼리에서 MANAGER_ID를 얻어와 서브쿼리에서
                                    -- 같은 EMP_ID를 가진 사원의 이름을 조회
FROM EMPLOYEE E1;                     
...
-- 해결 2) 해결 1의 조회 결과를 FROM절에 사용한 후 상위 5행만 조회하기
SELECT ROWNUM, EMP_NAME, SALARY FROM (SELECT EMP_NAME, SALARY
                                      FROM EMPLOYEE

...
/* SELECT * FROM EMPLOYEE 
   WHERE MONTHS_BETWEEN(SYSDATE, HIRE_DATE) / 12 >= 20; -- 주석은 내가 푼 답!
*/
...
/* SELECT EMP_NAME, JOB_CODE, 
    TO_CHAR(SALARY * 12 + 57000000, 'L999,999,999') || '원' AS "연봉(원)" 
FROM EMPLOYEE;
*/
...
FROM EMPLOYEE;
WHERE DEPT_CODE = 'D6'
ORDER BY JOB_CODE; -- 정렬 기준이 되는 컬럼은 반드시 SELECT절에 포함될 필요가 없음

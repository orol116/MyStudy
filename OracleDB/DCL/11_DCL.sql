...
CREATE USER yjb_sample IDENTIFIED BY sample1234;
--          사용자 계정 명           비밀번호

-- 2. 생성된 계정에 접속하기 위한 접속 방법을 추가(왼쪽 상단 초록색 + 버튼)
--> ORA-01045 : CREATE SESSION 권한을 가지고 있지 않아 로그인 실패

-- 3. [SYS 관리자 계정] 생성된 샘플 계정에 DB 접속 권한을 부여
GRANT CREATE SESSION TO yjb_sample; -- Grant을(를) 성공했습니다.
...
--> DB 접속 권한만 있고, 테이블 생성 권한은 없음

-- 6. [SYS 관리자 계정] 테이블 생성 권한 부여 + 객체 생성 공간 할당
GRANT CREATE TABLE TO yjb_sample; -- Grant을(를) 성공했습니다.

ALTER USER yjb_sample DEFAULT TABLESPACE SYSTEM QUOTA UNLIMITED ON SYSTEM;
...
-- [SYS 관리자 계정]
GRANT CONNECT, RESOURCE TO yjb_sample;

--------------------------------------------------------------------------------

-- 객체 권한
--> yjb_sample 계정에서 yjb 계정의 EMPLOYEE 테이블 조회

-- 1. [SAMPLE 계정] yjb계정의 EMPLOYEE 테이블 조회
SELECT * FROM yjb.EMPLOYEE; --> EMPLOYEE 테이블(객체) 조회 권한 없음

-- 2. [이니셜 계정] 샘플 계정에 EMPLOYEE 테이블 조회 권한 부여
GRANT SELECT ON EMPLOYEE TO yjb_sample; -- Grant을(를) 성공했습니다.

-- 3. [SAMPLE 계정] 다시 EMPLOYEE 테이블 조회
SELECT * FROM yjb.EMPLOYEE;

-- 4. [이니셜 계정] 부여했던 SELECT 권한 회수
REVOKE  SELECT ON EMPLOYEE FROM yjb_sample;

-- 5. [SAMPLE 계정] 다시 EMPLOYEE 테이블 조회
SELECT * FROM yjb.EMPLOYEE; --> 조회 안됨

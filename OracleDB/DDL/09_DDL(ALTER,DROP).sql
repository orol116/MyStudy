...
ALTER TABLE DEPT_COPY
MODIFY DEPT_TITLE NOT NULL; -- NOT NULL은 MODIFY 사용!! (꼭 기억하기)
...
-- ALTER TABLE DEPT_COPY DROP CONSTRAINT SYS_C00940 -- NOT NULL
...
-- (추가)
-- DEPT_COPY 테이블에 LNAME VARCHAR(30) 기본값 '한국' 컬럼 추가
ALTER TABLE DEPT_COPY
...
-- COMMIT / ROLLBACK 없이 구문을 수행하게 되면
...
--> 제약조건까지 모두 삭제
-- == FK 제약조건으로 인해 삭제가 원래는 불가능하지만, 제약조건을 없애버려서 FK 관계를 해제

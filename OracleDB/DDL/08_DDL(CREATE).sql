...
-- DML : 데이터 조작 언어(테이블에 데이터를 삽입, 수정, 삭제)
-- TCL : 트랜잭션 제어 언어(데이터 변경 사항을 DB에 반영, 되돌리기)
-- DQL : 데이터 질의 언어(데이터 조회)


-- DDL(DATA DEFINITION LANGUAGE) : 데이터 정의 언어(객체 생성, 수정, 삭제)
...
CREATE TABLE MEMBER(
    MEMBER_ID VARCHAR2(20), -- 가변 길이 문자열 20바이트(영어, 숫자만 작성 시 20글자 가능)
...
--> 접속창 -> 계정열고 -> 테이블 클릭 -> 위쪽 새로고침 -> MEMBER 테이블 보임
...
CREATE TABLE USER_USED_NN(
    USER_NO NUMBER NOT NULL, --사용자 번호(학번같은 느낌, 무조건 다들 있어야함 == NULL X)
                    --> 컬럼레벨 제약조건 설정
    USER_ID VARCHAR2(20) ,
...
-- ORA-01400: NULL을 ("YJB"."USER_USED_NN"."USER_NO") 안에 삽입할 수 없습니다
...
-- ORA-00001: 무결성 제약 조건(YJB.USER_ID_U)에 위배됩니다
...
--> 기본키 중복으로 오류
-- ORA-00001: 무결성 제약 조건(YJB.USER_NO_PK)에 위배됩니다 == UNIQUE와 같음

INSERT INTO USER_USED_PK
VALUES(NULL, 'user03', 'pass03', '유관순', '여', '010-9999-3131', 'yoo123@kh.or.kr');
--> 기본키가 NULL 이므로 오류
-- ORA-01400: NULL을 ("YJB"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다 == NOT NULL과 같음
...
-- ORA-00001: 무결성 제약 조건(YJB.PK_USERNO_USERID)에 위배됩니다
...
CREATE TABLE USER_USED_FK(
  ...
  PHONE VARCHAR2(30),
  EMAIL VARCHAR2(50),
  GRADE_CODE NUMBER -- CONSTRAINT GRADE_CODE_FK REFERENCES USER_GRADE -- 컬럼레벨 FK 설정
  ,
  CONSTRAINT GRADE_CODE_FK FOREIGN KEY(GRADE_CODE) REFERENCES USER_GRADE    
                                                    --> 참조 테이블 컬럼명 생략 시 자동으로 PK를 참조
...
--> 50이라는 값은 USER_GRADE 테이블 GRADE_CODE 컬럼에서 제공하는 값이 아니므로
 ...
 -- ORA-02291: 무결성 제약조건(YJB.GRADE_CODE_FK)이 위배되었습니다- 부모 키가 없습니다
            --> 참조 무결성 위배
...
WHERE GRADE_CODE = 10;
-- ORA-02292: 무결성 제약조건(YJB.GRADE_CODE_FK)이 위배되었습니다- 자식 레코드가 발견되었습니다
...
CREATE TABLE USER_USED_FK3(
  ...
  CONSTRAINT GRADE_CODE_FK3 FOREIGN KEY(GRADE_CODE) REFERENCES USER_GRADE3(GRADE_CODE) 
  ON DELETE CASCADE
...
-- ORA-02290: 체크 제약조건(YJB.GENDER_CHECK)이 위배되었습니다

...
SELECT * FROM DEPARTMENT2;
--> 조건을 설정하지 않아 모든 행의 컬럼 값이 수정되는 문제가 발생
---> WHERE절 잘 작성하기!!!
...
UPDATE EMPLOYEE2 SET BONUS = 0.3
...
SELECT * FROM EMP_M02; -- 5행 (4행 중복, 1핵 중복 X)
...
-- 사번이 겹치면 2번 테이블 테이터로 덮어쓰기
-- 사번이 겹치지 않으면 2번 테이블 내용 삽입하기

...
public class MemberView { // 회원 관련 화면 입출력
    ...
    public void signUp() {
        ...
        try {
            ...
            Member sighUpMember = new Member(memberId, memberPw, memberName, memberGender);

            // 회원 가입 Service 호출 후 결과 반환 받기
            // - 회원 가입 == DB에 회원 정보 삽입 == INSERT(DML)
            //   -> DML 구문 수행 시 성공한 행의 개수가 반환됨 == int형 변수로 결과를 저장
            int result = service.sighUp(sighUpMember);
    ...
    public void myInfo(Member loginMember) {
        ...
        if (loginMember.getMemberGender() == 'M') System.out.println("성별 : 남성");
            ...
            if (memberList.isEmpty()) System.out.println("조회 결과가 없습니다.");
                ...
                for (Member member : memberList) {
                    System.out.printf("%12s   %12s   %s\n",
                            member.getMemberId(), member.getMemberName(), member.getEnrollDate());
                }
            }
    ...
    public void updatePw(Member loginMember) {
        ...
        try {
                int result = service.updatePw(loginMember.getMemberNo(), newPw1, currentPw);

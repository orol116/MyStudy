...
public class MemberServiceImpl implements MemberService {
	...
	public Member login(Member inputMember) {
		...
		// ex) A 회원 / 비밀번호 1234 -> 암호화 : abcd
		// ex) B 회원 / 비밀번호 1234 -> 암호화 : @azd
		
		// * 매번 암호화되는 비밀번호가 달라져서 DB에서 직접 비교가 불가능하다.
		//   대신 Brypt 암호화를 지원하는 객체가
		//   이를 비교하는 기능(메서드)을 가지고 있어 이를 활용하면 된다.
...
//		logger.debug(inputMember.getMemberPw() + " / " + bcrypt.encode(inputMember.getMemberPw()));
		...
		if (loginMember != null) { // 일치하는 이메일을 가진 회원 정보가 있을 경우
			...
			// 입력된 비밀번호(평문), 조회된 비밀번호(암호화) 비교
			// bcrypt.matches(평문, 암호화) 순으로 작성해야함
			if (bcrypt.matches(inputMember.getMemberPw(), loginMember.getMemberPw())) { // 비밀번호가 일치할 경우
				...
				// => 비밀번호는 세션에 올리면 안된다!!!!!
				// => 나중에  loginMember는 세션에 올라가기 때문에 null로 비워주기
				
			} else { // 비밀번호가 일치하지 않을 경우
				loginMember = null; // 로그인을 실패한 형태로 바꿔준다.
			}
		}

...
//	}
	...
	public String signUp(Member inputMember
		...
		// 커맨드 객체를 이용해서 입력된 회원 정보를 잘 받아온다.
		// 단, 같은 name을 가진 주소가 하나의 문자열로 합쳐서 세팅되어있다.
		// -> 도로명 주소에 " , " 기호가 포함되는 경우가 있어 이를 구분자로 사용할 수 없다.
		
		// String[] memberAddress 
		// -> name이 memberAddress인 파라미터의 값을 모두 배열에 담아서 반환한다.
		
		inputMember.setMemberAddress(String.join(",,", memberAddress)); 
		// String.join("구분자", 배열)
		// 배열을 하나의 문자열로 합치는 메서드
		// 중간에 들어가는 구분자를 지정할 수 있다.
	...
	// 회원 1명 정보 조회(ajax)
	@ResponseBody // 반환되는 값이 forward/redirect 경로가 아닌 값 자체임을 인식
	...
	/* Spring 예외 처리 방법 (3가지, 중복 사용 가능)
	 * 
	 * 1 순위 : 메서드 별로 예외처리하기 (try - catch / throws)
	 * 
	 * 2 순위 : 하나의 컨트롤러에서 발생하는 예외를 모아서 처리하기 
	 * 			-> @ExceptionHandler (메서드에 작성)
	 * 
	 * 3 순위 : 전역(웹 애플리케이션)에서 발생하는 예외를 모아서 처리
	 * 			-> @ControllerAdvice (따로 만든 예외처리용 클래스에 작성)
	 */
	
	// 회원 컨트롤러에서 발생하는 모든 예외를 모아서 처리하기

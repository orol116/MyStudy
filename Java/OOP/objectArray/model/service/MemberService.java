...
public class MemberService {
    ...
    public MemberService() { // 기본 생성자
        ...
        memberArr[1] = new Member("user02", "pass02", "홍길순", 20, "인천");
        memberArr[2] = new Member("user03", "pass03", "홍길만", 40, "경기");
    }

    // 메뉴 출력용 메서드
    public void displayMenu() {

        int menuNum = 0; // 메뉴를 입력 받기 위한 변수
        ...
        do { // 한 번은 무조건 반복
            ...
            switch (menuNum) {
                ...
                case 4:
                    ...
                    if (result == -1) System.out.println("로그인 후 이용 해주세요.");
                    else if (result == 0) System.out.println("회원 정보 수정 실패(비밀번호 불일치)");
                    else System.out.println("회원 정보가 수정되었습니다.");
                    break;
    ...
    public int emptyIndex() {
        ...
        for (int i = 0; i < memberArr.length; i++) {
            ...
            if (memberArr[i] == null) return i; // 현재 메서드를 종료하고 호출한 곳으로 i 값을 가지고 돌아감
        }
    ...
    public String login() {
        ...
        // 1) memberArr 베열 내 요소를 순서대로 접근하여 null이 아닌지 확인
        for (int i = 0; i < memberArr.length; i++) {
